# EDC 0.16.0-Based Extension

The manuscript targets Eclipse EDC 0.16.0 PolicyEngine extension behavior. Current EDC documentation describes policy evaluation as extension code registered as policy functions, where functions evaluate an operator, right operand, rule, and policy context.

This repository now compiles against the actual Eclipse EDC 0.16.0 artifacts published to Maven Central:
...
The tests instantiate EDC 0.16.0 `PolicyEngineImpl`, `RuleBindingRegistryImpl`, `ScopeFilter`, and `RuleValidator` from EDC artifacts and evaluate S1 allow/block behavior through registered ODRL-KR functions.
